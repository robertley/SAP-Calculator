...
export class AbilityService {
    ...
    triggerSummonEvents(summonedPet: Pet) {
        ...
        for (let pet of this.abilityQueueService.getTeam(parent)) {
            if (pet == summonedPet) {
                this.abilityQueueService.triggerAbility(pet, 'ThisSummoned', summonedPet);
            } else {
                this.abilityQueueService.triggerAbility(pet, 'FriendSummoned', summonedPet);
                this.abilityQueueService.handleNumberedCounterTriggers(
                    pet,
                    summonedPet,
                    undefined,
                    this.abilityQueueService.getNumberedTriggersForPet(pet, 'FriendSummoned')
                );
                // Special summon types
                if (summonedPet.name === 'Bee') {
                    this.abilityQueueService.triggerAbility(pet, 'BeeSummoned', summonedPet);
                }
            }
        }
        ...
        enemyPets.forEach((pet: Pet) => this.abilityQueueService.triggerAbility(pet, 'EnemySummoned', summonedPet));
    ...
    triggerFoodEvents(eatingPet: Pet, foodType?: string) {
        ...
        for (let pet of this.abilityQueueService.getTeam(eatingPet)) {
            ...
            this.abilityQueueService.triggerAbility(pet, 'FoodEatenByFriendly', eatingPet);
            if (pet == eatingPet) {
                this.abilityQueueService.triggerAbility(pet, 'FoodEatenByThis', eatingPet);
                // Handle specific food types for this pet
                if (foodType === 'apple') {
                    this.abilityQueueService.triggerAbility(pet, 'AppleEatenByThis', eatingPet);
                    this.abilityQueueService.handleNumberedCounterTriggers(
                        pet,
                        eatingPet,
                        undefined,
                        this.abilityQueueService.getNumberedTriggersForPet(pet, 'AppleEatenByThis')
                    );
                } else if (foodType === 'corn') {
                    this.abilityQueueService.triggerAbility(pet, 'CornEatenByThis', eatingPet);
                }
                this.abilityQueueService.handleNumberedCounterTriggers(
                    pet,
                    eatingPet,
                ...
                this.abilityQueueService.triggerAbility(pet, 'FoodEatenByFriend', eatingPet);
                // Specific food types for friends
                if (foodType === 'corn') {
                    this.abilityQueueService.triggerAbility(pet, 'CornEatenByFriend', eatingPet);
                }
            }
        }

        // Check enemies
        for (let pet of this.abilityQueueService.getTeam(eatingPet.parent?.opponent)) {
            this.abilityQueueService.triggerAbility(pet, 'FoodEatenByAny', eatingPet);
        }
    }
    ...
    triggerPerkGainEvents(perkPet: Pet, perkType?: string) {
        ...
        for (let pet of this.abilityQueueService.getTeam(perkPet)) {
            this.abilityQueueService.triggerAbility(pet, 'FriendlyGainsPerk', perkPet);
            if (perkType === 'Strawberry') {
                this.abilityQueueService.triggerAbility(pet, 'FriendlyGainedStrawberry', perkPet);
            }
            if (pet == perkPet) {
                this.abilityQueueService.triggerAbility(pet, 'ThisGainedPerk', perkPet);
                // Special perk types
                if (perkType === 'Strawberry') {
                    this.abilityQueueService.triggerAbility(pet, 'ThisGainedStrawberry', perkPet);
                }
            } else {
                this.abilityQueueService.triggerAbility(pet, 'FriendGainsPerk', perkPet);
                // Special perk types for friends
                if (perkType === 'Strawberry') {
                    this.abilityQueueService.triggerAbility(pet, 'FriendGainedStrawberry', perkPet);
                }
            }
        }
    }
    ...
    triggerPerkLossEvents(perkPet: Pet, perkType?: string) {
        ...
        for (let pet of this.abilityQueueService.getTeam(perkPet)) {
            ...
            if (pet == perkPet) {
                ...
                // Special perk types for friends
                if (perkType === 'strawberry') {
                    this.abilityQueueService.triggerAbility(pet, 'FriendLostStrawberry', perkPet);
                }
            }
        }

        // Check enemies
        for (let pet of this.abilityQueueService.getTeam(perkPet.parent?.opponent)) {
            this.abilityQueueService.triggerAbility(pet, 'PetLostPerk', perkPet);
        }
    }
    ...
    triggerAilmentGainEvents(ailmentPet: Pet, ailmentType?: string) {
        ...
        for (let pet of this.abilityQueueService.getTeam(ailmentPet)) {
            this.abilityQueueService.triggerAbility(pet, 'AnyoneGainedAilment', ailmentPet);
            if (pet == ailmentPet) {
                this.abilityQueueService.triggerAbility(pet, 'ThisGainedAilment', ailmentPet);
            } else {
                this.abilityQueueService.triggerAbility(pet, 'FriendGainsAilment', ailmentPet);
            }
            // Special ailment types
            if (ailmentType === 'weak') {
                this.abilityQueueService.triggerAbility(pet, 'AnyoneGainedWeak', ailmentPet);
            }
        }

        // Check enemies
        for (let pet of this.abilityQueueService.getTeam(ailmentPet.parent?.opponent)) {
            this.abilityQueueService.triggerAbility(pet, 'EnemyGainedAilment', ailmentPet);
            this.abilityQueueService.triggerAbility(pet, 'AnyoneGainedAilment', ailmentPet);
            // Special ailment types
            if (ailmentType === 'weak') {
                this.abilityQueueService.triggerAbility(pet, 'AnyoneGainedWeak', ailmentPet);
    ...
    triggerTransformEvents(originalPet: Pet) {
        ...
        for (let pet of this.abilityQueueService.getTeam(transformedPet)) {
            if (pet == transformedPet) {
                this.abilityQueueService.triggerAbility(pet, 'ThisTransformed', transformedPet);
            } else {
                this.abilityQueueService.triggerAbility(pet, 'FriendTransformed', transformedPet);
                this.abilityQueueService.handleNumberedCounterTriggers(
                    pet,
                    transformedPet,
                    undefined,
                    this.abilityQueueService.getNumberedTriggersForPet(pet, 'FriendTransformed')
                );
            }
        }
    }
    ...
    triggerFlungEvents(movedPet: Pet) {
        ...
        for (let pet of this.abilityQueueService.getTeam(movedPet.parent?.opponent)) {
            this.abilityQueueService.triggerAbility(pet, 'AnyoneFlung', movedPet);
        }
    }

    triggerPushedEvents(pushedPet: Pet) {
        for (let pet of this.abilityQueueService.getTeam(pushedPet.parent?.opponent)) {
    ...
    triggerKillEvents(killerPet: Pet, killedPet: Pet) {
        ...
        if (killedPet.parent !== killerPet.parent) {
            this.abilityQueueService.triggerAbility(killerPet, 'ThisKilledEnemy', killedPet);
        }
    }
    ...
    triggerHurtEvents(hurtedPet: Pet, damageAmount?: number): void {
        ...
        for (let pet of this.abilityQueueService.getTeam(hurtedPet)) {
            this.abilityQueueService.triggerAbility(pet, 'AnyoneHurt', hurtedPet, customParams);
            if (pet == hurtedPet) {
                this.abilityQueueService.triggerAbility(pet, 'ThisHurt', hurtedPet, customParams);
                this.abilityQueueService.handleNumberedCounterTriggers(
                    pet,
                    hurtedPet,
                    customParams,
                ...
                this.abilityQueueService.triggerAbility(pet, 'FriendHurt', hurtedPet, customParams);
                this.abilityQueueService.handleNumberedCounterTriggers(
                    pet,
                    hurtedPet,
                    customParams,
            ...
            if (pet == pet.petBehind() || pet.petAhead) {
                this.abilityQueueService.triggerAbility(pet, 'AdjacentFriendsHurt', hurtedPet, customParams);
            }
            if (pet.position < hurtedPet.position) {
                this.abilityQueueService.triggerAbility(pet, 'AnyoneBehindHurt', hurtedPet, customParams)
            }
        }
        // check Enemies
        for (let pet of this.abilityQueueService.getTeam(hurtedPet.parent?.opponent)) {
            this.abilityQueueService.triggerAbility(pet, 'EnemyHurt', hurtedPet, customParams)
            this.abilityQueueService.triggerAbility(pet, 'AnyoneHurt', hurtedPet, customParams);
            this.abilityQueueService.handleNumberedCounterTriggers(
                pet,
                hurtedPet,
    ...
    triggerLevelUpEvents(levelUpPet: Pet) {
        ...
        for (let pet of player.petArray) {
            ...
            this.abilityQueueService.triggerAbility(pet, 'FriendlyLeveledUp', levelUpPet);
            if (pet == levelUpPet) {
                this.abilityQueueService.triggerAbility(pet, 'ThisLeveledUp', levelUpPet);
            } else {
                this.abilityQueueService.triggerAbility(pet, 'FriendLeveledUp', levelUpPet);
                this.abilityQueueService.handleNumberedCounterTriggers(
                    pet,
                    levelUpPet,
                    undefined,
                    this.abilityQueueService.getNumberedTriggersForPet(pet, 'FriendlyLeveledUp')
                );
            }
        }
        ...
        for (let pet of this.abilityQueueService.getTeam(jumpPet)) {
            ...
            if (pet == jumpPet) {
                ...
                this.abilityQueueService.handleNumberedCounterTriggers(
                    ...
                    jumpPet,
                    undefined,
                    this.abilityQueueService.getNumberedTriggersForPet(pet, 'FriendJumped')
                );
            }
        }

        // Check enemies
        for (let pet of this.abilityQueueService.getTeam(jumpPet.parent?.opponent)) {
            this.abilityQueueService.triggerAbility(pet, 'AnyoneJumped', jumpPet);
        }
