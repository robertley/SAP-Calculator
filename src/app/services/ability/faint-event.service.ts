...
export class FaintEventService {
    ...
    triggerFaintEvents(faintedPet: Pet) {
        ...
        for (let pet of team) {
            if (pet == faintedPet) {
                this.abilityQueueService.triggerAbility(pet, 'BeforeThisDies', faintedPet);
            } else {
                this.abilityQueueService.triggerAbility(pet, 'KitsuneFriendDies', faintedPet);
            }
            // Check for FriendAheadDied (pet ahead of the dying pet)
            if (pet == faintedPet.petBehind(null, true)) {
                this.abilityQueueService.triggerAbility(pet, 'FriendAheadDied', faintedPet);
            }
            // Check for AdjacentFriendsDie
            if (pet == faintedPet.petBehind(null, true) || pet.petBehind(null, true) == faintedPet) {
                this.abilityQueueService.triggerAbility(pet, 'AdjacentFriendsDie', faintedPet);
            }
        }
    }
    ...
    triggerAfterFaintEvents(faintedPet: Pet) {
        ...
        if (faintedPet.parent) {
            this.toyEventService.triggerFriendFaintsToyEvents(faintedPet.parent, faintedPet);
        }
        ...
        for (let pet of team) {
            ...
            if (pet == faintedPet) {
                this.abilityQueueService.triggerAbility(pet, 'ThisDied', faintedPet);
            } else {
        ...
        // Check enemies
        const enemyTeam = this.abilityQueueService.getTeam(faintedPet.parent?.opponent).filter(p => p.alive);
