...
import { processEventQueue, executeEventWithTransform } from "./event-queue-helpers";
...
export class AbilityQueueService {
    ...
    triggerAbility(pet: Pet, trigger: AbilityTrigger, triggerPet?: Pet, customParams?: any): void {
        if (pet.hasTrigger(trigger)) {
            ...
            const abilityEvent: AbilityEvent = {
                callback: (trigger: AbilityTrigger, gameApi: GameAPI, triggerPet: Pet) => {
                    pet.executeAbilities(trigger, gameApi, triggerPet, undefined, undefined, eventCustomParams)
                },
