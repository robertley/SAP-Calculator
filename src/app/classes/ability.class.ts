...
export class Ability {
  ...
  constructor(config: {
    ...
    const initialUses =
      config.initialCurrentUses ??
      (config.abilityType === 'Pet' ? config.owner.triggersConsumed : 0);
  ...
  execute(
    ...
    try {
      ...
      if (this.abilityType === 'Pet') {
        this.owner.triggersConsumed = Math.max(
          this.owner.triggersConsumed,
          this.currentUses,
        );
      }
      ...
      console.error(
        `Error executing ability ${this.name || 'unnamed'}:`,
        error,
      );
      return false;
    }
  ...
  tigerCheck(tiger?: boolean, tigerPetOverride?: Pet): boolean {
    ...
    if (
      tigerPet &&
      tigerPet.hasTrigger(undefined, undefined, 'TigerAbility') &&
      (tiger == null || tiger == false)
    ) {
      return true;
    }
    return false;
