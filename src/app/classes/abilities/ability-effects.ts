...
export function transformPetWithLog(options: {
    ...
    logAbilityEvent(options.logService, options.owner, options.context, options.message, options.extras);
...
export function awardExperienceWithLog(options: {
    ...
    const message = options.message ?? `${options.owner.name} gave ${options.target.name} +${options.amount} experience.`;
    logAbilityEvent(options.logService, options.owner, options.context, message, options.extras);
