...
export class HippogriffAbility extends Ability {
    ...
    private applyTemporaryBuff(target: Pet, context: AbilityContext, randomEvent: boolean): void {
        const owner = this.owner;
    ...
    private summonNextTierPet(gameApi: GameAPI, context: AbilityContext): void {
        ...
        const summonResult = owner.parent.summonPet(pet, owner.savedPosition, false, owner);
    ...
    override copy(newOwner: Pet): HippogriffAbility {
        return new HippogriffAbility(newOwner, this.logService, this.abilityService, this.petService);
    }
}
