...
export class PangasiusAbility extends Ability {
    ...
    private executeAbility(context: AbilityContext): void {
        ...
        const effectDescriptions = ['copied', 'copied with double effect', 'copied with triple effect'];
        const effectNote = effectDescriptions[Math.min(copies, effectDescriptions.length) - 1];
    ...
    private getFriendWithAilment(player: Player): Pet | null {
        const candidates = player.petArray
            .filter(pet => pet.alive && pet.equipment && this.isAilmentEquipment(pet.equipment))
