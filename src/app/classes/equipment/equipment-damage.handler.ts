...
const ABILITY_REDUCTION_CONFIGS: AbilityReductionConfig[] = [
    ...
    { name: 'NurikabeAbility', label: 'Nurikabe', perLevel: 4, incrementUses: true },
    { name: 'FanMusselAbility', label: 'Fan Mussel', perLevel: 1, incrementUses: true },
...
export class EquipmentDamageHandler {
    static applyDamage(options: EquipmentDamageOptions): void {
        const { pet, baseDamage, perkName, manticoreMultipliers, logService, afterDamage } = options;
    ...
    private static applyAbilityReductions(pet: Pet, damage: number): {
        damage: number;
