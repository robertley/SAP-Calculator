...
describe('Nessie roll count across simulations', () => {
    it('uses the configured roll amount on the second simulation', () => {
        ...
        expect(firstBattleMessage).toContain(
            `Nessie spawned a Nessie? ${expectedStats}`,
        );
        expect(firstBattleMessage).toContain(
            `It's the first Boat, so it gains ${expectedBonus}!`,
        );

        expect(secondBattleMessage).toContain(
            `Nessie spawned a Nessie? ${expectedStats}`,
        );
        expect(secondBattleMessage).toContain(
            `It's the first Boat, so it gains ${expectedBonus}!`,
